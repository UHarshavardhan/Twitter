...
const update = async(req,res)=>{
    ...
    if(newPassword=='' || currentPassword == ''){
        res.status(400).json({error:"Please prvide both the passwords"});
    }

      if(newPassword && currentPassword){
        const check= compare(currentPassword,user.password);
        if(!check){
            res.status(400).json({error:"paswords dosent match "});
        }
        else{
